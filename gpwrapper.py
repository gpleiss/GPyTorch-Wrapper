...
class GaussianProcess(object):
    ...
    def initialize_criterion(self, X, y):
        ...
        if not inspect.isclass(self.criterion): # if criterion has already been initialized
            self.criterion_ = self.criterion
            return
        if issubclass(self.criterion, gpytorch.mlls.VariationalMarginalLogLikelihood):
            if y is not None:
                self.criterion_ = self.criterion(self.likelihood_, self.module_, n_data=len(y))
            else:
                # X is a Dataset
                self.criterion_ = self.criterion(self.likelihood_, self.module_, n_data=len(X))
            ...
            print("unrecognizable criterion!")
        return self

    def initialize_likelihood(self):
        if isinstance(self.likelihood, gpytorch.likelihoods.Likelihood): # already initialized
        ...
        if self.likelihood == gpytorch.likelihoods.BernoulliLikelihood:
            ...
            print("Unrecognizable likelihood!")
    ...
    def initialize_module(self, X, y):
        ...
        elif issubclass(self.module, gpytorch.models.grid_inducing_variational_gp.GridInducingVariationalGP):
            self.module_ = self.module()
        elif issubclass(self.module, gpytorch.models.additive_grid_inducing_variational_gp.AdditiveGridInducingVariationalGP):
            self.module_ = self.module()
        else:
    ...
    def initialize_scheduler(self):
        if inspect.isclass(self.scheduler):
            self.scheduler_ = torch.optim.lr_scheduler.MultiStepLR(self.optimizer_, milestones=[0.75 * self.max_epochs], gamma=0.1)
        else:
    ...
    def predict(self, X):
        ...
        if issubclass(self.likelihood_.__class__, gpytorch.likelihoods.BernoulliLikelihood):
            return self.predict_proba(X).mean().detach().ge(0.5).float().mul(2).sub(1)
        elif issubclass(self.likelihood_.__class__, gpytorch.likelihoods.SoftmaxLikelihood):
